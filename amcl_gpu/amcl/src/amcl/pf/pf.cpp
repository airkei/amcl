...
{
  ROS_INFO("[PF]: pf_update_converged");
  int i;
  pf_sample_set_t *set;
  pf_sample_t *sample;
  double total;
...
{
  ROS_INFO("[PF]: pf_update_sensor IN");
  int i;
  pf_sample_set_t *set;
  pf_sample_t *sample;
  double total;
  ...
  ROS_INFO("[PF]: pf_update_sensor OUT");
...
{
  ROS_INFO("[PF]: pf_update_resample IN");
  int i;
#ifndef AMCL_USE_GPU
  double total;
#else
...
#else
  c = (float*)malloc(sizeof(float)*(set_a->sample_count+1));
#endif
  c[0] = 0.0;
  ...
  */
  ...
  free(c);
  ROS_INFO("[PF]: pf_update_resample OUT");
  return;
}
...
{
  ROS_INFO("[PF]: pf_get_cluster_stats");
  pf_sample_set_t *set;
  pf_cluster_t *cluster;
